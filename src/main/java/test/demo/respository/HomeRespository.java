...
public interface HomeRespository extends JpaRepository<Home,Long> {
    ...
    @Query("SELECT h.id,h.ten, h.diachi,h.giatien FROM Home h WHERE h.listingtypes LIKE %:listingtypes% AND h.offertype LIKE %:offertype% AND h.selectcity LIKE %:selectcity% ")
